...
import gaz_ptp.tsp_network as tsp_network
...
class Network(BaseNetwork):
    def __init__(self, config: BaseConfig, device: torch.device = None):
        ...
        self.situation_net = tsp_network.SituationNetwork(config=self.config, device=self.device)
    ...
    def forward(self, x: Dict):
        ...
        """
        boards = x
        situation_vector_batch, remaining_nodes_transformed_batch = self.situation_net(boards["current"])

        policy_logits_padded_masked, values = self.prediction_net(
            situation_vector=situation_vector_batch,
            remaining_nodes=remaining_nodes_transformed_batch,
    ...
    def player_situations_to_batch(situations: List[Dict], config: BaseConfig, to_device: torch.device = None,
        ...
        batch_dict = tsp_network.Network.player_situations_to_batch(situations, config, to_device,
                                                                     custom_max_seq_len)
    ...
    def states_batch_dict_to_device(batch_dict: Dict, to_device: torch.device):
        return tsp_network.Network.states_batch_dict_to_device(batch_dict, to_device)
...
class PredictionNetwork(nn.Module):
    ...
    def __init__(self, config: BaseConfig, device: torch.device = None):
        ...
        self.value_feedforward = nn.Sequential(
            nn.Linear(input_dim, self.latent_dimension),
            nn.GELU(),
    ...
    def forward(self, situation_vector, remaining_nodes, baseline,
